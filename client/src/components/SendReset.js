...
class SendReset extends Component {
  ...
  handleSubmit = (e) => {
    ...
    if (this.state.email) {
      ...
      fetch('/authentification/send-reset', {
        method: 'POST',
      ...
      .then(res => res.json()).then((res) => {
        if (res.success)
          ...
          this.setState({ working: false, notVerified: res.notVerified, errorMessage: res.message });
      });
    }
  }
        ...
        {this.state.errorMessage && (
          <div class="alert alert-danger" role="alert">
            ...
            <p>{this.state.errorMessage}</p>
              {this.state.notVerified && <p>Please check your spams for a confirmation email or try <Link to="/resend"> resending it</Link></p>}
          </div>)}
        <div id="sendResetContainer" className="rounded">
